...
/**
 * Creates a flag actor that will be used by harold
 * \returns The flag actor
 */
std::shared_ptr<CActor> CHaroldFactory::CreateFlag()
{
	shared_ptr<CActor> actor = make_shared<CActor>(L"Flag");

	// Creates the flag image
	auto flag = make_shared<CImageDrawable>(L"Flag", L"images/flag.png");
	actor->SetRoot(flag);
	flag->SetCenter(Point(127, 220));
	flag->SetRotation(3.14159 / 10);
