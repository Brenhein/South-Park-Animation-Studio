...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
	...
	ON_COMMAND(ID_ADDTREE_TREE1, &CMainFrame::OnAddTree1)
	ON_COMMAND(ID_ADDTREE_TREE2, &CMainFrame::OnAddTree2)
	ON_COMMAND(ID_PROPS_HARVEST32805, &CMainFrame::OnHarvest)
	ON_COMMAND(ID_PROPS_EMPTY32804, &CMainFrame::OnEmpty)
END_MESSAGE_MAP()
...
/**
 * Handles harvesting tree
 */
void CMainFrame::OnHarvest()
{
	int currentFrame = mPicture->GetTimeline()->GetCurrentFrame();

	for (auto actor : *mPicture) // iterates over actors
	{
		actor->GetRoot()->Harvest(currentFrame);
	}
	mPicture->UpdateObservers();
}
...
/**
 * Handles adding tree 1
 */
void CMainFrame::OnAddTree1()
{
	if (!mPicture->IsTree1()) // Tree 1 doesn't exist
	{
		mPicture->CreateTreeActor(1);
	}
}


/**
 * Handles adding tree 2
 */
void CMainFrame::OnAddTree2()
{
	if (!mPicture->IsTree2()) // Tree 2 doesn't exist
	{
		mPicture->CreateTreeActor(2);
	}
}

/**
 * Handles emptying basket
 */
void CMainFrame::OnEmpty()
{
	for (auto actor : *mPicture) // Iterates over the actor
	{
		for (auto child : *actor->GetRoot()) // iterates over the children 
		{
			child->EmptyBasket();
		}
	}
